...
\TG{We usually try to avoid consecutive title without text in between (there are 3 of them below, 4 including the chapter title).}
...
To offset the cost of disk I/O, the Linux kernel implements a disk cache \TG{it's not only used for disk I/O, I would remove ``disk" from this sentence}, 
called \textit{page cache}, by storing in memory data that requires 
disk accesses. 
There are two reasons that make the disk caches \TG{page cache?} important to 
operating systems. 
...
\english{With memory bandwidth, which is much faster than disk bandwidth, 
combined with the situations that data can be accessed in memory instead 
of on disk,} the I/O performance can be \english{markedly} improved. 
The Linux page cache is a part of RAM, which includes physical pages 
referring to pages on disk. 
The size of the page cache is dynamic \english{when} it can grow when there is 
available free memory, and can be shrunk to release memory if needed. 
Data cached in the page cache consists of pages, which means files \english{must 
...
\english{In writing,} the behavior of the page cache is more sophisticated.
...
Accessing these written pages may result in cache hits\english{, should the pages 
remain in memory}.
...
The third strategy is the approach implemented in Linux kernel \TG{isn't writethrough also implemented?},
...
After being written back to \english{backing store} \TG{permanent storage?}, these pages are no longer dirty 
...
a bulk write at a later time \cite{linuxdev3rd2010}. \TG{Add a line on the drabacks of writeback.}

\subsubsection{Caching on NFS}

\TG{It's unclear if this section refers to NFS as a specific implementation (e.g., NFSv4) or as a general technology that includes Lustre and other network file systems.
If you're referring to the latter I think you should avoid using the NFS acronym.}
...
\english{In reading on NFS}, if data is cached \english{on client}, data is read from client cache 
...
However, \english{there is no server cache in writing} since the data written to 
...
If server cache \english{was enable} for writing, a server crash during a cache 
write \english{could result in a problem that the client could not be aware of whether} 
...
the NFS server with an old version of \english{file begin written}. 
Thus, only writethrough strategy can be implemented \TG{should would be better than can. Can means that 
it's not possible to implement writeback. It is possible but it's not recommended.} for writing on NFS. 
...
to evict. \english{According to the idea of} the page cache, data that is more likely 
to be accessed in the future should be kept in page cache, and the data 
that is least likely to be used should be evicted. 

We have \english{mentioned about} the page cache, \english{adding pages to the page cache, 
cache eviction}. But \english{how the page cache is exactly structured, how data is 
...
\english{data references of a process tend to cluster}. Thus, it selects \english{pages that 
has} not been referred for \english{a longest} time to remove 
...
When a page is referenced, if it is not in the page cache, \english{it then be added 
...
\english{Now assume that in a particular scenario, when a user is working in a 
...
the page cache instead disk, which makes reading time way faster.  \TG{Not sure how this example shows the interest of LRU/2 vs LRU.}
...
\begin{itemize}
    ...
    number of dirty pages has reached its limit because \TG{why because?} only clean pages are 
    available for eviction. \TG{split in two conditions?}
...
which defines a percentage of total available memory \TG{a percentage that is used for what?}. 
...
\english{There is a fact that} real-world platforms are not built for 
...
This may \english{result in the reproducibility problem of the results} since 
...
Last but not least, users and researchers can be \english{limited to} platform 
...
Furthermore, \TG{this paragraph seems to be truncated.}
...
The simulation frameworks are designed \english{with the goal is to} provide 
...
in computer science. \TG{You should also comment on the drawbacks and challenges of simulations compared to real experimentation.}
...
In off-line simulation, system events logged \english{with time-stamped} are retrieved 
...
In the previous section \TG{not in the previous, you should just refer to section numbers} we have mentioned that simulation models are 
a component in most simulation frameworks.
The simulation models and abstractions are implemented \english{to aid the 
...
\english{Over the years,} simulators developed and used by researchers in 
...
%     \vspace*{-0.7cm}
     \caption{Overview and main components in \simgrid framework \TG{I don't think this figure is yours, you should explicitly write ``figure extracted from [x]''. Please check all the other figures too. }}
%     \vspace*{0.5cm}
     \label{fig:simgrid}
\end{figure*}
...
Figure~\ref{fig:wrench} shows the software architecture of \wrench. 
...
The next layer implements CI \TG{define acronym} services that are commonly
...
models that yield high accuracy \TG{this repeats text from previous sections, you should avoid that}, as demonstrated by
